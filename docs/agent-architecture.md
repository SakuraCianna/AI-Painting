...
- `backend/app/agent/plantuml_edit_planner.py`: PlantUML 源码级语音编辑计划器, 当前支持节点改名/删除、甘特任务追加/删除、泳道追加/删除和 ER 关系新增/改写
...
- 已知 PlantUML 编辑先用本地源码编辑计划, 例如把流程图节点改名或删除、给甘特图追加或删除任务、给泳道图追加或删除泳道、给 ER 图新增关系或改写关系基数
...
- 用真实 ASR transcript 和图片 Provider 样本持续回归 108 条复杂语音评测集, 不再只看规则文本样本
- 扩展领域工具: 室内、人物、看板图、Mermaid / PlantUML 和海报版式
- 扩展 PlantUML 支持范围到更多可参数化 UML 关系、移动节点、关系端点重连和更细的关系基数编辑
