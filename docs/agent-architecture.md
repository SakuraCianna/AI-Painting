...
- `backend/app/agent/edit_planner.py`: 语义编辑计划器, 将“把沙发改成绿色并向右移动一点”“把屋顶下面的门改成绿色”“把靠近门的那棵树改成黄色”“把卡片里的文字改成蓝色”拆成 `set_style_many`、`move_many` 等受控操作
- `backend/app/repositories.py`: 对象查询 DSL 执行层, 支持排序选择、相对位置、靠近关系、遮挡关系、包含关系、同一行/同一列、层级前后、颜色温度、小物件筛选和组级扩展
...
- 已知语义编辑先用本地 Agent 编辑计划, 例如改沙发颜色并移动、改流程图节点颜色并加粗箭头、编辑屋顶下面的门、靠近门的树、挡住标题的图片、卡片里的文字、和标题同一行的按钮或暖色小物件
...
- 扩展更细的语义关系选择, 例如组内相邻对象、最近对象、路径附近对象和关系链组合查询
