...
- `backend/app/agent/planner.py`: Drawing Agent Planner, 负责启用条件、本地模板、流程图模板、信息图模板、海报模板、UI 草图模板和 Graph 调度
...
- 简单命令继续走规则解析, 不调用模型
- 已知复杂模板先用本地 Agent 模板, 例如客厅场景、语音绘图流程图、销售增长信息图、新品发布海报和产品 UI 草图
