...
- `backend/app/agent/planner.py`: Drawing Agent Planner, 负责启用条件、本地模板、语义编辑模板、流程图模板、系统架构图模板、ER 图模板、自定义泳道和节点的泳道图模板、信息图模板、海报模板、UI 草图模板、自定义角色组织结构图模板、甘特图模板和 Graph 调度
...
- 已知复杂模板先用本地 Agent 模板, 例如客厅场景、语音绘图流程图、系统架构图、用户订单 ER 图、自定义泳道和节点的泳道图、销售增长信息图、新品发布海报、产品 UI 草图、自定义角色的产品团队组织结构图和项目排期甘特图
