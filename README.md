...
- 支持生成图片局部精修, 例如“把人物肖像的眼睛精修一下”“把右边那张生成图的天空精修一下”, 后端会把源图和原始提示词一起交给图生图模型
...
8. 对已生成图片的局部精修会先用 `prompt_contains`、`position`、`rank` 或 `corner` 定位图片对象, 再把源图 `src`、原始 `prompt`、目标区域和用户新指令合成图生图提示词
9. 局部精修结果会记录 `source_prompt`、`source_object_id` 和 `target_region`, 方便继续语音追改
