...
- 支持对象查询 DSL、组级语义选择和高级相对关系第一版, 例如“把左边第二棵树改成黄色”“把屋顶下面的门改成绿色”“把靠近门的那棵树改成黄色”“把挡住标题的图片向右移动一点”“把卡片里的文字改成蓝色”“把和标题同一行的按钮改成绿色”
...
11. 对象查询 DSL 已支持排序选择、相对位置、靠近关系、遮挡关系、包含关系、同一行/同一列、层级前后、颜色温度、小物件筛选和组级扩展, 例如左边第 N 个对象、屋顶下面的门、靠近门的树、挡住标题的图片、卡片里的文字、和标题同一行的按钮、暖色小物件、整棵树和整个沙发
