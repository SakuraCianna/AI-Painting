...
- **PlantUML 语音查看**: 支持“放大这张架构图”“聚焦支付模块”“还原图表视图”等纯语音查看命令, 前端只调整 PlantUML 图层的只读视图状态, 不修改源码, 也不新增鼠标编辑入口。
...
- **ASR 多级兜底**: 小米 MiMo ASR 优先, 本地 Qwen3-ASR 和 Web Speech API 作为备用路径。后端 ASR 支持 WebSocket 音频流式上传, 静音截停后返回最终转写；Web Speech API 兜底路径支持浏览器 interim 文本。Provider 状态接口会返回能力声明, 前端控制台会显示当前是“流式上传 · 整段识别”还是“实时 interim”。
...
| PlantUML 图层查看 | 已支持 | “放大这张架构图”“聚焦支付模块”“还原图表视图” |
...
PlantUML 图层的缩放和聚焦是前端只读查看状态: “放大这张架构图”会放大当前 PlantUML 图层, “聚焦支付模块”会根据 PlantUML 源码中的模块声明估算焦点区域并居中放大, “还原图表视图”会回到完整图层视图。这些命令不会提交后端绘图编辑计划, 不会改写 PlantUML 源码或对象数据, 也不会增加鼠标拖拽、滚轮缩放或手动框选入口。
