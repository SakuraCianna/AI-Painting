...
- 支持对象查询 DSL、组级语义选择、高级相对关系和关系链组合查询第一版, 例如“把左边第二棵树改成黄色”“把屋顶下面的门改成绿色”“把靠近门的那棵树改成黄色”“把挡住标题的图片向右移动一点”“把卡片里的文字改成蓝色”“把卡片里和标题同一行的按钮改成绿色”
...
- 支持生成图片局部精修, 例如“把人物肖像的眼睛精修一下”, 后端会把源图和原始提示词一起交给图生图模型
...
11. 对象查询 DSL 已支持排序选择、相对位置、靠近关系、遮挡关系、包含关系、同一行/同一列、层级前后、关系链组合、颜色温度、小物件筛选、生成图提示词匹配和组级扩展, 例如左边第 N 个对象、屋顶下面的门、靠近门的树、挡住标题的图片、卡片里的文字、卡片里且和标题同一行的按钮、暖色小物件、整棵树、整个沙发和人物肖像生成图
...
4. “精修图片”“丰富当前画面”“风格转换”会走图生图精修, 使用当前画布截图和提示词一起生成覆盖式图片层；如果用户指定已生成图片的局部, 后端会优先读取该图片对象的 `src` 和原始 `prompt`
...
8. 对已生成图片的局部精修会先用 `prompt_contains` 定位图片对象, 再把源图 `src`、原始 `prompt`、目标区域和用户新指令合成图生图提示词
