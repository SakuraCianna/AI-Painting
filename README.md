...
- 支持复合语音指令按整条命令分组撤销和恢复, 例如一次撤销“三颗星星”或“改色后移动”
- 支持最近对象缩放, 例如“把它放大一倍”
- 支持局部语义对象形状替换, 例如“把窗户改成圆形”
- 支持空间语义筛选, 例如“把左边窗户改大一点”
- 支持对象查询 DSL、组级语义选择、高级相对关系和关系链组合查询第一版, 例如“把左边第二棵树改成黄色”“把屋顶下面的门改成绿色”“把靠近门的那棵树改成黄色”“把挡住标题的图片向右移动一点”“把卡片里的文字改成蓝色”“把卡片里和标题同一行的按钮改成绿色”“把第二张生成图精修一下”
...
- 支持生成图片局部精修, 例如“把人物肖像的眼睛精修一下”“把右边那张生成图的天空精修一下”“把右边那个人的眼睛调亮”, 后端会把源图、原始提示词、目标主体、目标区域和调整动作一起交给图生图模型
- 支持渲染策略路由, 结构精确类图形走程序生成, 艺术表现类图形走生图模型
- 支持 Drawing Agent + SceneGraph v2 复杂规划入口, 例如“画一个温馨客厅, 有沙发、茶几、窗户和落地灯”
- 支持 Agent 语义编辑计划, 例如“把客厅的沙发改成绿色并向右移动一点”“把流程图节点改成浅蓝色, 同时把箭头加粗”
- 绘图引擎会校验操作类型、对象类型、图层 ID、缩放因子和替换形状, 避免异常计划污染画布历史
...
当前项目处于 MVP 后产品化扩展阶段。它已经打通语音识别、指令解析、结构化绘图、持久化、复合撤销恢复和确认链, 但距离商业可用的纯语音绘图工具仍需要补齐真实 ASR 评测、开放式复杂场景规划、无鼠标端到端验收和延迟 SLO。
...
- `.github/workflows/ai-painting-ci.yml`: 在 Pull Request、任意分支推送和手动触发时运行后端覆盖率、前端覆盖率、前端构建和 API smoke test
- CI 还会校验 Docker Compose 配置并构建备用部署镜像
- `.github/workflows/cd.yml`: 在 `main` 推送、`v*` tag 和手动触发时构建发布包, tag 触发时自动创建 GitHub Release

CI 使用 Python `3.12.10` 和 `.node-version` 指定的 Node.js `24`。后端覆盖率门槛为总覆盖率 `85%`, 前端覆盖率门槛为行、语句和函数 `85%`, 分支 `70%`。CD 产物包含后端源码、前端 `dist`、项目文档和 `快速启动.bat`。
...
9. 当前已落地本地客厅场景、流程图、信息图、海报、UI 草图、组织结构图和甘特图 Agent 模板, 并新增第一版语义编辑计划
10. Agent 语义编辑支持沙发、窗户、流程图节点、箭头、甘特图任务条、海报标题、UI 按钮等常见目标的改色、移动、缩放、加粗和形状替换
11. 对象查询 DSL 已支持排序选择、相对位置、靠近关系、遮挡关系、包含关系、同一行/同一列、层级前后、关系链组合、颜色温度、小物件筛选、生成图提示词匹配、生成图第 N 张选择、生成图四角选择和组级扩展, 例如左边第 N 个对象、屋顶下面的门、靠近门的树、挡住标题的图片、卡片里的文字、卡片里且和标题同一行的按钮、暖色小物件、整棵树、整个沙发、人物肖像生成图、右边那张生成图和右上角生成图
12. 绘图引擎仍会做最终安全校验, 即使绕过 Agent 直接提交异常操作, 也会拒绝未知对象、非法缩放和不支持的形状替换
...
4. “精修图片”“丰富当前画面”“风格转换”会走图生图精修, 使用当前画布截图和提示词一起生成覆盖式图片层；如果用户指定已生成图片的局部, 后端会按提示词、位置、第 N 张或四角位置优先读取该图片对象的 `src` 和原始 `prompt`
...
8. 对已生成图片的局部精修会先用 `prompt_contains`、`position`、`rank` 或 `corner` 定位图片对象, 再把源图 `src`、原始 `prompt`、目标主体、目标区域、调整动作和用户新指令合成图生图提示词
9. 局部精修结果会记录 `source_prompt`、`source_object_id`、`target_subject`、`target_region` 和 `adjustment`, 方便继续语音追改
10. 用户可以用“撤销”移除精修图, 回到可编辑源画布
...
后端会把多步骤语音指令作为一个计划执行。计划执行时只清理一次 redo 栈, 并在所有步骤成功后统一提交 SQLite 事务。如果任一步失败, 会回滚整次计划, 避免画布进入半完成状态。成功执行后, 同一条语音指令内的多步操作会共享 `command_group_id`, 撤销和恢复会按整条语音命令回退或重放。
