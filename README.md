...
- **ASR 多级兜底**: 小米 MiMo ASR 优先, 本地 Qwen3-ASR 和 Web Speech API 作为备用路径。后端 ASR 当前是静音截停后的整段转写, Web Speech API 兜底路径支持浏览器 interim 文本, 后端流式实时转录仍在 Roadmap。Provider 状态接口会返回能力声明, 前端控制台会显示当前是“整段转写”还是“实时 interim”。
...
- 后端 ASR 仍是整段音频提交, 尚未接入 WebSocket 或增量流式转写。
