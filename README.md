...
- **ASR 多级兜底与指令纠错**: 小米 MiMo ASR 优先, 本地 Qwen3-ASR 和 Web Speech API 作为备用路径。后端 ASR 支持 WebSocket 音频流式上传, 静音截停后返回最终转写；执行前会对高置信 ASR 误识别做领域化纠错, 例如把“学生管理系统的一样”修复成“学生管理系统的ER图”。Web Speech API 兜底路径支持浏览器 interim 文本。Provider 状态接口会返回能力声明, 前端控制台会显示当前是“流式上传 · 整段识别”还是“实时 interim”。
...
| 能力 | 状态 | 示例 |
| --- | --- | --- |
| 基础图形 | 已支持 | 圆形、矩形、椭圆、三角形、线条、箭头、星形、多边形、路径、贝塞尔、菱形、平行四边形、梯形、十字形、爱心、月牙、圆环、圆柱 |
