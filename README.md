...
- **PlantUML 源码级编辑**: 支持通过语音修改和删除图表节点、追加或删除甘特任务、追加或删除泳道, 以及新增开放 ER 实体节点、按关系标签或实体端点新增、删除和改写 ER 关系。带图表结构上下文的节点/实体改名不依赖固定词表；新增 ER 实体会优先按领域规则推断关系, 未命中规则时会连接到当前图里关系最集中的实体。
- **矢量场景优先**: 房子、草地、太阳、树和简单场景走程序化 SVG 渲染, 便于精确修改。后续追加房子、云朵、树、花、太阳、月亮和普通基础图形时会经过布局约束层, 根据天空区、地面区、已有主体和碰撞评分选择位置, 尽量放到语义合理区域并避免遮挡主体。
- **轻量布尔几何**: 支持 `boolean_shape` 图形对象, 用 `base/add/subtract` 操作组合基础形状, 当前用于参数化月相、圆环、云朵、花朵和对话气泡。
- **规则解析槽位层**: 基础图形的颜色、形状、数量、N 边形和 N 瓣花解析集中在 `backend/app/command_slots.py`, 避免为单个说法写死局部规则。
- **生图增强**: 水墨画、二次元人物、商业视觉图等艺术类任务可以走 GPT-image-2 或 OpenAI 兼容 Provider。
- **图生图精修**: 支持基于上一张图片和追改指令继续精修, 例如“把右边那个人的眼睛调亮”；也支持“把这个画换成 Minecraft 风格的图片”这类当前画布整体风格转换, 前端会附带画布截图。
- **复杂指令拆解**: Drawing Agent 能把复杂语音拆成结构化步骤, 再由执行器逐步落到画布。
- **安全确认链**: 清空画布等高风险操作保留 `requires_confirmation`, 确认后才真正执行。
- **复合撤销**: 一条语音生成的多步操作可以整体撤销和恢复。
- **ASR 多级兜底与指令纠错**: 小米 MiMo ASR 优先, 本地 Qwen3-ASR 和 Web Speech API 作为备用路径。后端 ASR 支持 WebSocket 音频流式上传, 静音截停后返回最终转写；执行前会对高置信 ASR 误识别做领域化纠错, 通过上下文规则把“学生管理系统的一样”“库存管理的一样”等 ER 后缀误识别修复成“...的ER图”。Web Speech API 兜底路径支持浏览器 interim 文本。Provider 状态接口会返回能力声明, 前端控制台会显示当前是“流式上传 · 整段识别”还是“实时 interim”。
...
| 能力 | 状态 | 示例 |
| --- | --- | --- |
| 基础图形 | 已支持 | 圆形、矩形、椭圆、三角形、线条、箭头、星形、多边形、路径、贝塞尔、菱形、平行四边形、梯形、十字形、爱心、圆柱, 以及布尔几何月相、圆环、云朵、N 瓣花、对话气泡；支持常见颜色同义词、N 边形和重复基础图形 |
...
| PlantUML 图表编辑 | 第一版 | “把流程图里的ASR识别节点改成语音识别”“删除甘特图里的测试任务”“把ER图里用户和订单之间的关系改成一对一的下单关系” |
| 开放场景构图 | 第一版 | “画一个公园场景, 有草地、太阳、两棵树、一条小路和长椅” |
| 文生图 | Provider 链路已支持 | “生成一张二次元动漫人物” |
| 图生图精修 | Provider 链路已支持 | “继续把他的头发柔和一点”“把这个画换成 Minecraft 风格的图片” |
...
删除甘特图里的测试任务
删除泳道图里的运营泳道
把ER图里的创建关系改成一对一的下单关系
把ER图里用户和订单之间的关系改成一对一的下单关系
删除ER图里用户和商品之间的关系
画一个公园场景, 有草地、太阳、两棵树、一条小路和长椅
把左边第二棵树改成黄色
生成一张二次元动漫人物
精修我的图片
把这个画换成 Minecraft 风格的图片
清空画布
确认清空
撤销
恢复
...
PlantUML 图表会优先使用 `AI_PAINTING_PLANTUML_JAR` 本地渲染；如果没有显式配置, 后端会自动尝试 `backend/tools/plantuml.jar`。本地 jar 使用 UTF-8 管道输入并自动注入中文字体, SVG 会归一化为等比展示, 避免中文乱码和画面拉伸。生成的 PlantUML 模板使用较紧凑的默认字号, 小图不会被强行放大, 超出画布的大图会等比缩小。jar 渲染失败后会继续尝试 `AI_PAINTING_PLANTUML_SERVER_URL`。Docker Compose 会启动备用 `plantuml-server` 服务, 后端默认通过 `http://plantuml-server:8080` 调用。jar 和 server 都不可用时, 后端会生成安全的 SVG 源码预览图层, 不会阻断语音绘图流程。
...
- 后端 ASR 已支持 WebSocket 音频流式上传, 但小米和本地 Provider 仍是静音截停后的最终转写, 尚未实现 Provider 级 partial transcript。
