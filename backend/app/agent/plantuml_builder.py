...
def build_plantuml_scene_graph(text: str) -> AgentSceneGraph | None:
    ...
    if any(keyword in text for keyword in ("er图", "er 图", "实体关系图", "实体关系")):
        return _plantuml_er_graph(text)
    if any(keyword in text for keyword in ("系统架构", "技术架构", "应用架构", "架构图", "结构图")) and not any(
        keyword in text for keyword in ("组织架构", "团队架构", "组织结构")
    ):
    ...
    if any(keyword in text for keyword in ("类图", "uml图", "uml")):
